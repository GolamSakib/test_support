...
//api response data return
if (!function_exists('JsonDataResponse')) {
    function JsonDataResponse($data = null)
    {
        if ($data) {
...
if (!function_exists('billingOfCustomer')) {
    function billingOfCustomer($id)
    {
        $ch = curl_init();
        $url = 'http://software.mediasoftbd.com/msbill/api/BillCollection/GetCustomerWiseBillDetailsWithExpDate?cusID=' . $id;
        ...
        return $response = json_decode(curl_exec($ch))->data;
    }
}
...
if (!function_exists('humanReadableDate')) {
    function humanReadableDate($date)
    {
        ...
        // Format the date as "M d, Y" (e.g., Feb 27, 2023)
...
if (!function_exists('amountinWords')) {
    function amountinWords($amount)
    {
        ...
        $amountInWords = $numberToWords->format($amount);
...
if (!function_exists('monitoringOfCustomer')) {
    function monitoringOfCustomer($id)
    {
        $ch = curl_init();

        $url = 'http://software.mediasoftbd.com/msbill/api/CustomerMonitoring/GetCustomerMonitoringByCusID?CusID=' . $id;
...
//api response data return
if (!function_exists('saveDataResponse')) {
...
if (!function_exists('updateDataResponse')) {
...
//update response data return
if (!function_exists('deleteDataResponse')) {
...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        if (env('APP_ENV') == 'production') {
            $url = env('APP_URL'); //aws s3/cloudfront base url
            return $url . $path;
        }
...
if(!function_exists('formatDate')){
    function formatDate($dateString)
    {
            ...
            if ($epochTime === false) {
              $formattedDate = DateTime::createFromFormat('d-m-Y', $dateString) ?: 
                 DateTime::createFromFormat('m/d/Y', $dateString) ?: 
                 DateTime::createFromFormat('Y-m-d H:i:s', $dateString);
                
                if ($formattedDate) {
                    $epochTime = $formattedDate->getTimestamp();
                } else {
                    $epochTime = 'Invalid date';
                }
            }
